...
class REDQ:
    def __init__(self, state_dim, action_dim, max_action, device, num_critics=10, subset_size=2):  
        ...
        self.critic = Critic(state_dim, action_dim, max_action, num_critics=self.num_critics).to(device)
        # Target Critic network
        self.critic_target = Critic(state_dim, action_dim, max_action, num_critics=self.num_critics).to(device)
    ...
    def train(self, replay_buffer, batch_size, utd_ratio = 20, discount_factor = 0.99, tau = 0.005):
        ...
        for _ in range(utd_ratio): # Perform multiple updates per environnment step
            ...
            with torch.no_grad():
                ...
                subset_targets = all_targets[random_indices]
                min_target_Q, _ = torch.min(subset_targets, dim=0)
            ...
            critic_loss = 0
            for ind in range(self.num_critics):
            ...
            # Freeze Critic --> we do not need to compute the gradient for the critic when we update the actor.
            ...
            # Unfreeze Critic for the next loop
            for param in self.critic.parameters():
