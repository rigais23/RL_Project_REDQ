...
class Critic(nn.Module):
    ...
    def forward(self, state, action):
        ...
        # Stack all the tensor into a single tensor. 
...
class Actor(nn.Module):
    ...
    def forward(self, state):
        ...
        log_std = torch.clamp(log_std, min=-20, max=2) # Clamp log_std to prevent numerical issues
        return mean, log_std
    ...
    def sample(self, state):
        ...
        x_t = normal.rsample() # Reparameterization Trick --> reparameterized sample
